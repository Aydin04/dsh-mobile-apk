...
internal class GuidePageRenderer(private val activity: MainActivity) {
    ...
    Thread {
      ...
      activity.runOnUiThread {
        ...
        if (ok) {
          ...
          apkHint(result ?: "Download failed")
          toast(result ?: "Download failed")
        }
      }
    }.start()
  }
  ...
  private fun continueInstall() {
    ...
    if (!apk.exists()) {
      ...
      setUpdateButton(activity.getString(R.string.ds_check_update), enabled = true)
      apkHint("APK package not found, please check for updates again")
      return
    }
    ...
    if (UpdateChecker.invokeInstaller(activity, apk)) {
      ...
      apkHint("Failed to launch system installer, please tap button to retry")
    }
  }
